...
import { formatDuration, ICON, isLiveRun, QueryPlanData, readPlan, relativeDate, Tone, useDialog, useMediaQuery } from './lib'
...
function AgentPanel({ collectionId, onClose }: { collectionId?: string; onClose: () => void }) {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (send.isPending) return; const value = input.trim(); if (!value) return; const requestId = crypto.randomUUID(); setMessages((items) => [...items, { id: `user-${requestId}`, role: 'user', content: value }]); setInput(''); setPhase('Searching your knowledge'); setPlan(undefined); send.mutate({ content: value, requestId }) }
